...
class RestoreProgressRepository(private val context: Context) {
  ...
  /** Calls the notifyTooltipShown method on the connected service. */
  suspend fun notifyTooltipShown() {
    val service = currentService.get()
    if (service == null) {
    ...
    try {
      service.notifyTooltipShown()
      logger.atDebug("Notified service of tooltip shown")
    } catch (e: RemoteException) {
      logger.e("Failed to notify service of tooltip shown", e)
    }
  }
  ...
  private fun createRestoreProgressFlow(
    ...
    try {
      ...
      service.registerCallbackForProgressUpdatesWithVersion(
        callback,
        IRestoreProgressService.VERSION,
      )
    } catch (e: RemoteException) {
